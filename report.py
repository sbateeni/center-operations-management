...
def create_document():
    ...
    run_title = p_title.add_run("تقرير تفصيلي وشامل حول مشروع\nنظام غرفة العمليات الجغرافية وتتبع الحملات الميدانية\n(Geographical Operations Control Room)")
    ...
    add_body_paragraph("استجابة لهذه المتطلبات، يأتي مشروع \"نظام غرفة العمليات الجغرافية وتتبع الحملات الميدانية (Gemini-house-Navigator)\" ليوفر منصة قيادة وسيطرة متكاملة. يعتمد النظام على أحدث التقنيات الجغرافية وقواعد البيانات السحابية اللحظية (Supabase & React) لتمكين قادة العمليات من رؤية الوحدات وتتبع تحركاتها على الخرائط الرقمية بدقة عالية، مما يسهم في اتخاذ قرارات سريعة، مدروسة، ومبنية على بيانات حية من أرض الواقع.")
    ...
    add_body_paragraph("يمثل تطبيق \"نظام غرفة العمليات الجغرافية\" المستند على تقنيات حديثة خطوة جوهرية نحو التحول الرقمي وإدارة الموارد الشرطية بشكل احترافي وذكي. فهو يتجاوز كونه أداة تتبع ليصبح عقلاً إلكترونياً مساعداً يجمع أطراف مسرح العمليات أمام صانع القرار في شاشة واحدة تتفاعل لحظياً مع التطورات.")
    ...
    add_bullet_point("• تدريب الأطقم والمشغلين", "تدريب شامل لضباط غرفة العمليات على كيفية استخدام لوحة القيادة الاستراتيجية وإدارة الأزمات ونداءات الاستغاثة عبر النظام.")
